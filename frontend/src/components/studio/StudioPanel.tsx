...
export function StudioPanel() {
  ...
  if (!enabled || pathname.startsWith(ROUTES.admin)) return null
  ...
  return (
    <div className={`studio ${open ? 'studio--open' : ''}`}>
      {open && (
        <div className="studio__card">
          <div className="studio__row">
            <span className="studio__label">{CONTENT.studio.themeLabel}</span>
          ...
          <div className="studio__row">
            <span className="studio__label">{CONTENT.studio.layoutLabel}</span>
          ...
          <div className="studio__foot">
            <span>{overridden ? CONTENT.studio.overridden : CONTENT.studio.showingSaved}</span>
            {overridden && (
              <button className="studio__reset" onClick={reset}>
                {CONTENT.studio.reset}
              </button>
            )}
          </div>
        </div>
      ...
      <button className="studio__fab" onClick={() => setOpen((o) => !o)}>
        {open ? CONTENT.studio.fabClose : CONTENT.studio.fabOpen}
      </button>
    </div>
  )
}
