...
import { DEFAULT_THEME, isKnownTheme } from '@/theme/registry'
...
function readPersisted(): Persisted {
  try {
    return JSON.parse(localStorage.getItem(STORAGE_KEYS.studio) || '{}')
  } catch {
    return {}
  }
}
...
export function StudioProvider({
  ...
  // Per-store CSS variable overrides from store_config.
  useEffect(() => {
  ...
  useEffect(() => {
    if (overridden) localStorage.setItem(STORAGE_KEYS.studio, JSON.stringify(override))
    else localStorage.removeItem(STORAGE_KEYS.studio)
  }, [override, overridden])
